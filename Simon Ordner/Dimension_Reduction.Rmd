...
t_col <- function(color, percent, name = NULL) {
  ...
  t.col <- rgb(rgb.val[1], rgb.val[2], rgb.val[3],
             max = 255,
             alpha = (100 - percent) * 255 / 10000,
             name = name)

## Save the color
invisible(t.col)
}
mycol <- t_col(color = 'white', percent = 99.999, name = 'transparent')

# Create a named vector of colors including "Other"
color_palette <- c(setNames(level_colors, top_10_levels), Other = 'transparent')
```
...
```{r}
sapply(2:20,function(k){kmeans(breast_cancer_umap$layout, centers =k)$tot.withinss})
```
