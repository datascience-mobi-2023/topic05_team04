...
dim(prism.achilles.breast) #17 cell lines???

View(breast_prism.cnv)

View(prism.achilles.breast)

barcodes <- intersect(rownames(prism.achilles.breast), rownames(breast_prism.exp))

View(barcodes) #one code is the same, rest differs

barcodes1 <- intersect(rownames(prism.achilles.breast), rownames(breast_prism.cnv))
...
achilles.exp_breast <- data.frame(row.names = rownames(breast_prism.exp), stringsAsFactors = FALSE)


for (gene_name in colnames(prism.achilles.breast)) {
  achilles_value <- prism.achilles.breast[, gene_name]
  exp_value <- breast_prism.exp[, gene_name]
  achilles.exp_breast <- cbind(achilles.exp_breast, achilles_value, exp_value)
}
