...
for(i in 1:length(duplicate_drugs_b)) {
    ...
    if (length(act_rows_b) > 3) {
        act_reg_b <- lm(act_df_b$concentration_b ~ act_df_b$colMeans_breast, data = act_df_b)
        act_residuals_b <- residuals(act_reg_b)
        regression_list_b[[i]] <- list(summary(act_reg_b), act_residuals_b, summary(act_reg_b)$r.squared)
