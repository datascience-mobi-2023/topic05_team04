...
for(i in 1:length(duplicate_drugs_b)) {
    ...
    if (length(act_rows_b) > 3) {
        ...
        act_plot_b <- data.frame(x = act_df_b$concentration_b, y = act_df_b$colMeans_breast)
        regression_list_b[[i]] <- list(summary(act_reg_b), act_reg_b, act_residuals_b, summary(act_reg_b)$r.squared, act_plot_b)
...
prediction_tool <- function(drugname, conc_values) {
  postion <- which(drugname == names(regression_list_b))
  act_model_b <- regression_list_b[[position]][[2]]
  predictions <- predict(act_model_b, newdata = data.frame(conc_values))
  plot <- plot(regression_list_b[[position]][[5]]$x, regression_list_b[[position]][[5]]$y, pch = 16, col = "black", xlab = "Concentration", ylab = "log2FC"); abline(regression_list_b[[position]][[2]], col = "red")
