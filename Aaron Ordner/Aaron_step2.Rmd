...
sum(which("oncology"%in%prism.treat$disease.area))/dim(prism.treat)[1]
```
...
treat.names <- as.character(prism.treat$name)
sum(treat.names%in%"fulvestrant ")
print(cancer.therapy[309,])
print(as.character(prism.treat[74,]$name))
...
apply(rel.cl, 1, function(x){
  mean(prism.all.var[which(rownames(prism.all.var)%in%prism.cl$DepMap_ID[prism.cl$lineage%in%x])])
})

prism.all.var[which(rownames(prism.all.var)%in%prism.cl$DepMap_ID[prism.cl$lineage%in%rel.cl[1,]])]


which(rownames(prism.all.var)%in%prism.cl$DepMap_ID[prism.cl$lineage%in%rel.cl[1,]])
```
