...
cancer.therapy <- read_excel("~/Downloads/oncological_treatments.xlsx")
...
breast.ach.test <- ach.test.rem[rownames(prism.achilles.rm) %in% prism.cl[breast.cl, 1],]

breast.achilles.mean <- apply(breast.ach.test, 2, mean)
prism.achilles.other <-ach.test.rem[-which(rownames(prism.achilles.rm) %in% prism.cl[breast.cl, 1]), ]
other.achilles.mean <- apply(prism.achilles.other, 2, mean)

ach.low.pos <- which(breast_achilles_mean<other_achilles_mean)

hist(other_achilles_mean)

breast.ach.test <- breast.ach.test[,ach.low.pos]
prism.achilles.other <- prism.achilles.other[,ach.low.pos]
...
for(x in 1:length(ach.low.pos))  {
  result <- wilcox.test(breast.ach.test[, x], prism.achilles.other[, x], alternative = "less")
  achilles.genes <- rbind(achilles.genes, result$p.value)
  
}
rownames(achilles.genes) <- colnames(breast.ach.test)
achilles.test.genes <- which(achilles.genes<0.05)

#print(achilles.genes[19,3])
...
achilles.rel.genes <- which(breast_achilles_relmeans[achilles.low.score] < 0)




boxplot.data.breast <- as.data.frame(prism.achilles.breast[,552])
colnames(boxplot.data.breast) <- "ALG11"
boxplot.data.breast$lineage<- "breast"
boxplot.data.other <- as.data.frame(prism.achilles.rm[-breast.cl,552])
colnames(boxplot.data.other) <- "ALG11"
boxplot.data.other$lineage <- "other"
achilles.boxplot <- rbind(boxplot.data.breast, boxplot.data.other)


ggplot(achilles.boxplot)+
  geom_boxplot(aes(x = lineage, y = ALG11), fill = "lightgrey")+
  theme_minimal()+
  labs(title = "Prism.achilles scores for gene ALG11",
       x = "Lineage",
       y = "Achilles score",
       )+
...
achilles.genes.adj <- p.adjust(unlist(achilles.genes), method = "fdr")

min(achilles.genes.adj)

which(achilles.genes.adj<0.05)

rownames(achilles.genes)[c(311, 386)]

```
```{r}
rownames(which(breast.achilles.mean<(-1)))
```
...
w.genes.cancer <- achilles.rel.genes[which(names(achilles.rel.genes)%in%hist.targets$genes)]
