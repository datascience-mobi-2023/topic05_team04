...
ach.low.pos <- which(breast.achilles.mean<other.achilles.mean&breast.achilles.mean<0)
...
rownames(achilles.genes)[c( 206, 254)]
...
for (gene_name in w.genes.cancer) {
    ...
    new_row <- c(frequency_value, achilles_value, exp_value, p_value, treatments, mutation, hallmark)
...
colnames(Wgene.krebs) <- c("Frequency in list.targets", "mean prism.achilles", "mean prism.exp", "p-value from achilles.genes.adj", "treatments", "mutation", "hallmark")
...
```{r}
#junkyard
names.target[which(targets.treat$target==w.genes.cancer[2])]

paste(c("hallo", "wie", "gehts"), sep = ", ")

treatmentsrownames(Wgene.krebs) <- w.genes.cancer

names.target <- as.vector(targets.treat$name)

levels(targets.treat$name[1])
