...
silhouette_score1 <- function(k){
  km <- kmeans(prism_pca$x, centers = k)
  s <- silhouette(km$cluster, dist(prism_pca$x))
  mean(s[, 3])
}
k <- 2:20
avg_s1 <- sapply(k, silhouette_score1) # takes long to calculate
#best cluster size through silhouette method -> size 2 works best -> anyways 20 beacause moas
plot(k, type = 'b', avg_s1,xlim = c(2,20), ylim = c(min(avg_s1)-1,max(avg_s1)+1), xlab='Number of clusters', ylab='Average Silhouette Scores'); abline(h = max(avg_s1), col = 'red')
...
k <- 2:20
avg_s2 <- sapply(k, silhouette_score2)
# best cluster size through silhouette method -> size 3 works best -> trotzdem 20
plot(k, type = 'b', avg_s2,xlim = c(2,20), ylim = c(min(avg_s2)-1,max(avg_s2)+1), xlab='Number of clusters', ylab='Average Silhouette Scores'); abline(h = max(avg_s2), col = 'red')
