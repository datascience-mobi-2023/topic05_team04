...
PRISM & Profiling Relative Inhibition Simultaneously in Mixture \\
...
By addressing these challenges through drug repurposing, we can accelerate the availability of new treatment options for breast cancer and other diseases, making substantial progress in improving patient outcomes while maximizing resource efficiency.  
...
ggplot1 <- ggplot() +
    ...
    labs(title = "Lowest average prism score", x = "Concentration [nM]", y = "Count")
...
ggplot2 <- ggplot() +
    ...
    labs(title = "Highest average prism score", x = "Concentration [nM]", y = "Count")

grid.arrange(ggplot1, ggplot2, nrow = 1)
```

Once the most effective drug concentration combinations were identified, we proceeded with an additional round of filtering. This time only the drugs resulting in an average prism score lower than -2 were selected. This threshold was chosen because its allowing the removal of 60 % of the provided drugs. This can be seen in Figure 2. 

```{r, fig.pos= "H", echo = FALSE, message=FALSE, fig.align='center', fig.cap='Histogram of the mean log 2 fold change of the drugs after the first round of filtering. The colors indicate if the drugs is below or above the selected threshold of -2.', out.width='0.65\\linewidth'}
#### Plot 3
...
The initial goal was to utilize the UMAP technique to recreate the UMAP from Corsello *et al.*. In their paper, they used the selected and filtered prism dataset for further analysis of the prism scores of the treatments, with the MOA as the label for the data points. We employed Principal Component Analysis (PCA) on our version of the filtered prism dataset. However, upon examining the variance of the different principal components, we found that further analysis provided no new information and less information than Corsello *et al.* with their UMAP. The PCA plot showed that the data points were densely packed together, and only one cluster of tubulin polymerization inhibitors could be discerned.
We then applied the UMAP function to the same data, using the parameters mentioned in Corsello's paper: seven neighbors and a minimal distance of 0.5. Surprisingly, the resulting UMAP differed from Corsello *et al.*, despite using the same parameters. Similar to the UMAP created by Corsello *et al.*, MOAs tended to cluster together. By visually inspecting the plot, we could identify clusters of HSP inhibitors, tubulin polymerization inhibitors, and, to a lesser extent, HDAC inhibitors, MEK inhibitors, and glucocorticoid receptor agonists. This is depicted in Figure 5 in the Appendix.
Next, we applied K-means clustering to the UMAP, using the 20 MOAs we selected as labels for the data points as centres for the clustering despite three being the best centre number proven by the silhouette method. However, a limitation of this method is that with a large number of data points, the clusters tend to vary slightly with each run. As shown in Figure 3, the clusters included MOAs labeled as "Other" and even some data points from different MOAs. Among the previously mentioned clusters, only the cluster of tubulin polymerization inhibitors remained mostly intact. MEK inhibitors, HSP inhibitors, and HDAC inhibitors were more frequently divided between two clusters, and when they formed their own clusters, they shared those clusters with a few other treatments.

```{r, fig.pos= "H", fig.dim=c(12,6), echo = FALSE, message=FALSE, fig.align='center', fig.cap='UMAP of the inhibitory treatments with the PRISM scores labelled by the MOA', out.width='0.65\\linewidth'}
...
To begin, a correlation analysis between the gene expression data and the gene copy number data was conducted. The method used was the Pearson correlation. Our hypothesis was that the copy number would correlate highly with gene expression. This was important to see if the number of gene copies can be an indicator for gene expression.  Figure 6 presents the histogram of the correlation calculations, which demonstrates that, for the majority of genes, a positive correlation indeed exists. The mean correlation was approximately 0.325, with a median of 0.345. Yet, the correlation is not as high as was expected. Around 15.23% of the correlations are negative.
In the subsequent step, we constructed a correlation matrix using Pearson correlation to examine the relationship between gene expression data and treatment response data. This correlation matrix allowed us to further refine our understanding of which genes were associated with specific treatments. The resulting matrix contains correlations between 18,805 genes and 1,395 treatments. The data was then used in the further gene analysis.

### Statistical testing of important genes

A threshold of absolute correlation greater than 0.75 was applied to select genes for subsequent analysis. This threshold was selected as it is the mean of the highest absolute correlation for each treatment. The filtering resulted in selection of 3925 genes out of the total 18119 genes in the gene knockout data set. These genes were then sorted based on their gene knockout scores being lower than 0 and then a one-sided Wilcoxon rank sum test with significance level 0.05 was performed to assess if their mean scores were significantly lower than those of other lineages. As previous Shapiro-Wilk-Test showed the data is not normally distributed and therefore a non-parametric test was needed. The p-values were adjusted using the False Discovery Rate (FDR) correction. One gene showed a significant difference, SDHC with a p-value of 0.025. This is shwon in Figure 7 in the Appendix.

### Dataframe for targets involving genes

Moreover, the 3925 preselected genes were compiled into a data frame for further research. Genes with a mean gene knockout score below -1 for breast cell lines were included in the final data frame. 108 preselected genes fulfilled this criterion. This data frame includes information such as the frequency of high absolute correlation with a treatment, mean expression score, mean gene knockout score and the p-value from the Wilcoxon rank sum test. Furthermore, it provides additional details on gene mutations in breast cancer cell lines, treatments targeting the gene, and its association with any cancer hallmarks. The data on the cancer hallmarks has been taken from the Human Molecular Signatures Database of the Broad institute. The final data frame concludes our gene analysis as it combines our gene selection process with general information we gather about the genes along the way. Two genes showed a high frequency amongst treatments, mTOR and AURKA. These genes were associated with more than 20 treatments during the analysis.
...
prediction_tool <- function(drugname, conc_values) {
    ...
    ggplot <- ggplot(data = comb_df, aes(x = concentration_b, y = colMeans, color = level)) +
        ...
        geom_point() +
        labs(x = "Concentration [nM]", y = "Mean of PRISM score", title = paste("Regression for:", drugname), color = "") +
...
In hindsight, this approach may not be optimal to further decrease the number of potential targets. As stated before, correlation of gene expression does not imply the direct causality between the treatment and the gene expression. So, looking at the importance of a gene via gene knockout score does not show that a gene could be a good target. With more time to analyse the data, we would want to test new ways in looking into the data and selecting genes. Possible approaches could be as follows. It might improve the correlation analysis if we did it separately with each sub lineage of breast cancer. To detect potential targets, a correlation analysis between gene knockout score and gene expression could be used with further sub selection via negative mean knockout scores. The gene expression and treatment response correlation could be used as markers for potential new targeted therapy on base of individual gene expression. 
Finding such as the UMAP clustering could be investigated by applying the available data.


# References

Brancíková, D., Mechl, Z., Adam, Z., Jandáková, E., Pavlovský, Z., Válek, V., and Andrašina, Z. (2015). Patient with inoperable pheochromocytoma. Curr Oncol 22, e216-219.  
Cerqua, C., Buson, L., and Trevisson, E. (2021). Mutations in Assembly Factors Required for the Biogenesis of Mitochondrial Respiratory Chain. In Mitochondrial Diseases: Theory, Diagnosis and Therapy, P. Navas, and L. Salviati, eds. (Springer International Publishing), pp. 33-68.  
Chawla, S., Rockstroh, A., Lehman, M., Ratther, E., Jain, A., Anand, A., Gupta, A., Bhattacharya, N., Poonia, S., Rai, P., et al. (2022). Gene expression based inference of cancer drug sensitivity. Nature Communications 13, 5680.  
...
Gonçalves, E., Segura-Cabrera, A., Pacini, C., Picco, G., Behan, F.M., Jaaks, P., Coker, E.A., van der Meer, D., Barthorpe, A., Lightfoot, H., et al. (2020). Drug mechanism-of-action discovery through the integration of pharmacological and CRISPR screens. Molecular Systems Biology 16, e9405.  
Joshua, M.D., Jordan, R., Mariya, K., Joshua, P., Guillaume, K., David, E.R., and Aviad, T. (2019). Extracting Biological Insights from the Project Achilles Genome-Scale CRISPR Screens in Cancer Cell Lines. bioRxiv, 720243.  
Kumbhar, P., Kole, K., Manjappa, A., Jha, N.K., Disouza, J., and Patravale, V. (2023). Drug Repurposing Opportunities in Cancer. In Drug Repurposing for Emerging Infectious Diseases and Cancer, R.C. Sobti, S.K. Lal, and R.K. Goyal, eds. (Springer Nature Singapore), pp. 53-87.  
Lee, B.R., Yu, J.Y., Yoon, S.H., Ban, H.J., Kwon, Y.S., Oh, I.J., Kim, K.S., Kim, Y.I., Lim, S.C., and Kim, Y.C. (2015). Variability in the anti-tumor effect of tegafur-uracil depending on histologic types of lung cancer. J Thorac Dis 7, 433-438.  
Łukasiewicz, S., Czeczelewski, M., Forma, A., Baj, J., Sitarz, R., and Stanisławek, A. (2021). Breast Cancer-Epidemiology, Risk Factors, Classification, Prognostic Markers, and Current Treatment Strategies-An Updated Review. Cancers (Basel) 13.
...
Pushpakom, S., Iorio, F., Eyers, P.A., Escott, K.J., Hopper, S., Wells, A., Doig, A., Guilliams, T., Latimer, J., McNamee, C., et al. (2019). Drug repurposing: progress, challenges and recommendations. Nature Reviews Drug Discovery 18, 41-58.
Shao, X., Lv, N., Liao, J., Long, J., Xue, R., Ai, N., Xu, D., and Fan, X. (2019). Copy number variation is highly correlated with differential gene expression: a pan-cancer study. BMC Medical Genetics 20, 175.  
Sleire, L., Førde, H.E., Netland, I.A., Leiss, L., Skeie, B.S., and Enger, P.Ø. (2017). Drug repurposing in cancer. Pharmacological Research 124, 74-91.  
...
Yu, C., Mannan, A.M., Yvone, G.M., Ross, K.N., Zhang, Y.-L., Marton, M.A., Taylor, B.R., Crenshaw, A., Gould, J.Z., Tamayo, P., et al. (2016). High-throughput identification of genotype-specific cancer vulnerabilities in mixtures of barcoded tumor cell lines. Nature Biotechnology 34, 419-423.  
Zhu, K., Wu, Y., He, P., Fan, Y., Zhong, X., Zheng, H., and Luo, T. (2022). PI3K/AKT/mTOR-Targeted Therapy for Breast Cancer. Cells 11, 2508.


# Appendix

```{r, fig.pos= "H", fig.dim=c(12,6), echo = FALSE, message=FALSE, fig.align='center', fig.cap='UMAP of the inhibitory treatments with the PRISM scores labelled by the MOA with cluster encircelment', out.width='0.65\\linewidth'}
ggplot(umap_layout_prism, aes(x = V1, y = V2, color = levels, alpha = alpha_level)) +
    geom_point(size = 3) +
    scale_color_manual(values = color_palette0) +
    theme_bw() +
...
```{r, fig.pos= "H",warning = FALSE, echo = FALSE, message=FALSE, fig.align='center', fig.cap='Histogram of the correlation between gene expression and gene copy number. Mean and median are shown by red and blue dashed lines.', out.width='0.65\\linewidth'}
breast_prism.cl <- prism.cl[which(prism.cl$lineage == 'breast'), ]
...
ggplot(exp.copy.cor)+
  ...
  geom_vline(xintercept = 0.324, linetype = "dashed", color = "#BE3428FF", size = 1)+
  geom_vline(xintercept = 0.345, linetype = "dashed", color = "#088BBEFF", size = 1)+
  geom_text(x = 0.18, y = 500, label = "mean", color = "#BE3428FF", vjust = -1, size = 7) +
  geom_text(x = 0.52, y = 500, label = "median", color = "#088BBEFF", vjust = -1, size = 7) +
  theme_minimal()+
...
```{r, fig.pos= "H", echo = FALSE, message=FALSE, fig.align='center', fig.cap='Boxplott of the gene knockout scores for the gene SDHC for breast cancer lineages compared to all the other lineages', out.width='0.65\\linewidth'}
...
ggplot(achilles.boxplot)+
  ...
  theme_minimal()+
  labs(title = "Gene knockout scores for gene SDHC",
       x = "Lineage",
       y = "Achilles score",
       )+
  theme(plot.title = element_text(hjust = 0.5),
...
```{r, fig.pos= "H", echo = FALSE, message=FALSE, fig.align='center', fig.cap='Histogram of the correlation between the different concentrations and the respective average prism score. Correlations below the set threshold of -0.7 are colored red and correlations higher that -0.7 are colored blue.', out.width='0.65\\linewidth'}
####################################
...
```{r, fig.pos= "H", echo = FALSE, message=FALSE, fig.align='center', fig.cap='Histogram of the R² values for the linear regression models calculated for every drug between concentration and average prism score.', out.width='0.65\\linewidth'}
#### Plot 7
ggplot() +
...
```{r, fig.pos= "H", echo = FALSE, message=FALSE, fig.align='center', fig.cap='Histogram of the p-values resulting from the Shapiro wilk normality test on the residuals for the linear regression models calculated for every drug between concentration and average prism score. The treatments with a p-values below 0.05 are significant and are colored red.', out.width='0.65\\linewidth'}
#### Plot 8
